...
function slcanStringify(frame) {
    ...
    // Build SLCAN string
    const slcanStr = typeChar + idStr + lenStr + dataStr + '\r';
    return slcanStr;
}
...
export default class SlcanBus extends EventEmitter {
	...
	send(frame) {
		let s=slcanStringify(frame);
		if (!s)
			throw new Error("Not a CAN frame!");

		console.log("Sending from js: ",s);
		this.port.write(s+"\n");
	}
