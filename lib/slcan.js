export function slcanParse(s) {
    s = s.trim();
    if (s.length === 0) return undefined;

    const type = s[0];

    // Only handle t, T, r, R
    if (!['t', 'T', 'r', 'R'].includes(type)) {
        return undefined;
    }

    const isExtended = type === 'T' || type === 'R';
    const isRTR = type === 'r' || type === 'R';

    if (isExtended) {
        // Extended frame: TiiiiiiiiLdd... or RiiiiiiiiL
        if (s.length < 10) return undefined;

        const id = parseInt(s.substring(1, 9), 16);
        if (Number.isNaN(id)) return undefined;

        const len = parseInt(s[9], 16);
        if (Number.isNaN(len) || len < 0 || len > 8) return undefined;

        if (isRTR) {
            // RTR frame: RiiiiiiiiL
            if (s.length !== 10) return undefined;

            return {
                id,
                data: new Uint8Array(0),
                ext: true,
                rtr: true
            };
        } else {
            // Data frame: TiiiiiiiiLdd...
            const expectedLength = 10 + len * 2;
            if (s.length !== expectedLength) return undefined;

            const data = new Uint8Array(len);
            let offset = 10;

            for (let i = 0; i < len; i++) {
                const byte = parseInt(s.substring(offset, offset + 2), 16);
                if (Number.isNaN(byte)) return undefined;
                data[i] = byte;
                offset += 2;
            }

            return {
                id,
                data,
                ext: true,
                rtr: false
            };
        }
    } else {
        // Standard frame: tiiiLdd... or riiiL
        if (s.length < 5) return undefined;

        const id = parseInt(s.substring(1, 4), 16);
        if (Number.isNaN(id)) return undefined;

        const len = parseInt(s[4], 16);
        if (Number.isNaN(len) || len < 0 || len > 8) return undefined;

        if (isRTR) {
            // RTR frame: riiiL
            if (s.length !== 5) return undefined;

            return {
                id,
                data: new Uint8Array(0),
                ext: false,
                rtr: true
            };
        } else {
            // Data frame: tiiiLdd...
            const expectedLength = 5 + len * 2;
            if (s.length !== expectedLength) return undefined;

            const data = new Uint8Array(len);
            let offset = 5;

            for (let i = 0; i < len; i++) {
                const byte = parseInt(s.substring(offset, offset + 2), 16);
                if (Number.isNaN(byte)) return undefined;
                data[i] = byte;
                offset += 2;
            }

            return {
                id,
                data,
                ext: false,
                rtr: false
            };
        }
    }
}

export function slcanStringify(frame) {
    if (!frame || typeof frame.id !== 'number') {
        return undefined;
    }

    const id = frame.id;
    const isExtended = !!frame.ext;
    const isRTR = !!frame.rtr;

    let dataLength = 0;
    let dataBytes = [];

    if (!isRTR && frame.data != null) {
        const data = frame.data;

        if (ArrayBuffer.isView(data)) {
            dataLength = Math.min(data.length, 8);
            dataBytes = Array.from(data.slice(0, dataLength));
        } else if (Array.isArray(data)) {
            dataLength = Math.min(data.length, 8);
            dataBytes = data.slice(0, dataLength);
        } else {
            return undefined;
        }
    }

    // Frame type character
    const typeChar = isRTR
        ? (isExtended ? 'R' : 'r')
        : (isExtended ? 'T' : 't');

    // Format ID
    const idStr = isExtended
        ? id.toString(16).padStart(8, '0').toUpperCase().slice(-8)
        : id.toString(16).padStart(3, '0').toUpperCase().slice(-3);

    // Data length
    const lenStr = dataLength.toString(16).toUpperCase();

    // Data bytes
    let dataStr = '';
    if (!isRTR && dataLength > 0) {
        dataStr = dataBytes
            .map(b => b.toString(16).padStart(2, '0').toUpperCase())
            .join('');
    }

    return typeChar + idStr + lenStr + dataStr;
}
